                        ...
                        <div class="col-xl-12">
                            <div class="card easion-card">
                                ...
                                <?php 
                                    ...
                                    $id = $_GET['id'] ?? '';
                                    $sql = "SELECT * FROM Curso WHERE codCurso =  $id";
                                    $dados = mysqli_query($conn, $sql);
                                    $linha = mysqli_fetch_assoc($dados);

                                ?>
                                    <form name="curso" action="../editar/processar/Editar_Curso.php" method="POST">
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-6">
                                                <label for="inputDevice">Departamento</label>
                                                <select name="codDepart" id="codDepart" class="form-control">
                                                    <option value="1" selected ><?php echo $linha['codDepart'] ?></option>
                                                </select>
                                            </div>
                                            <input type="hidden" name="id" value="<?php echo $linha['codCurso'] ?>">
                                        </div>
                                        <div class="container-fluid">
                                            <button type="submit" class="btn btn-primary">Salvar</button> 
                                            <a class="dropdown-item" href="../exibir/Exibir_Curso.php" style="display: contents;"><button type="button" class="btn btn-primary">Visualizar Cadastros</button></a>
                                        </div>
                                    </form>
                                </div>   
                            </div>
